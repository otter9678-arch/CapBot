...
namespace CapBot.TaskTests
{
    internal static class AdaptiveLearningTests
    {
        ...
        internal static int Run()
        {
            ...
            // at the 25th; failed share 25/25 => adversity rule.
            string c = Id(22, true);
            SeedNeutral(c);
            ...
            Check(AdaptiveLearningDirector.GetRecord(c).CrossingCount == 1, "LEARN02b crossing consumed (per-record; global counter also holds (a)'s silent crossing)");
            ...
            Check(AdaptiveLearningDirector.ExpiredCount == 3, "LEARN07 hygiene decayed the stale records (y1 too — stale at sweep time)");
            Check(HasLineContaining("LearningRecordExpired LEARN:" + y1), "LEARN07 expiry line (y1) emitted");
            ...
            Check(AdaptiveLearningDirector.HistoryCount == 3, "LEARN07 history holds the expired ids");
            ...
            // armed at the CURRENT level, no bogus adjustment.
            ...
            Check(ry1 != null && ry1.ArmedLevel == CrewExperienceRegistry.Get(y1).Level,
                "LEARN07 swept-then-refreshed record re-arms at current level");
