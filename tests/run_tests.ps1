...
& $csc -nologo -nologo -out:$exe `
  ...
  (Join-Path $repo 'CapBot\Core\Learning\TraitProfileDirector.cs') `
  (Join-Path $repo 'CapBot\Core\Persistence\CrewSaveCodec.cs') `
  ...
  (Join-Path $repo 'tests\TraitConsumerTests.cs') `
  (Join-Path $repo 'tests\CrewSaveTests.cs') 2>&1 | ForEach-Object { Write-Output $_ }
